...
function getParentStudents($id) {
	global $wpdb;

	$column = $wpdb->get_results( $wpdb->prepare(
		"SELECT * FROM {$wpdb->prefix}parents_students as PS
...
function getParentStudentsClass($id) {
	global $wpdb;
...
class Parents_List extends WP_Users_List_Table {
    ...
    public function get_parents( $per_page = 20, $page_number = 1 ) {
		...
		if ( ! empty(  $_REQUEST['s'] ) ) {
			...
			$student_search_key = !empty( $_REQUEST['student'] ) ? wp_unslash( trim( $_REQUEST['student'] ) ) : '';
			...
			if ($student_search_key != '') {
				$search_queries['s.name'] = $student_search_key;
			}

			if ($branch_search_key != '') {
				$search_queries['b.branch_id'] = $branch_search_key;
			}
	...
	public function display_rows() {
 	...
 	public function single_row( $user_object, $style = '', $role = '', $numposts = 0 ) {
	    ...
	    if ( current_user_can( 'list_users' ) ) {
	        ...
	        $edit_link = esc_url( '?page=parents_add&id='.$user_object['ID'].'' );
	        ...
	        if ( current_user_can( 'edit_user', $user_object['ID'] ) ) {
	            ...
	            $edit            = "<strong><a href=\"{$edit_link}\">{$user_fullname}</a>{$super_admin}</strong><br />";
	    ...
	    foreach ($students_data	 as $value) {
	    	if ( ($students_data_i == $students_data_len - 1) OR ($students_data_len == 1)  ) {
				//last or not only 1
	    		$students .= $value['name'].'<br/>('.($value['number']).')';
		    } else {
	    		$students .= $value['name'].'<br/>('.($value['number']).')<br/>';
		    }
	...
	public function no_items() {
		_e( 'No Parents available.', 'alifakids' );
	}
	...
	public function process_bulk_action() {
		...
		if ( 'delete' === $this->current_action() ) {
			...
			if ( ! wp_verify_nonce( $nonce, 'bulk-users' ) ) {
				...
				$this->delete_parents( absint( $_GET['user'] ) );
			}
		}
		...
		elseif ( ( isset( $_GET['action'] ) && $_GET['action'] == 'bulk-delete' )
			...
			foreach ( $delete_ids as $id ) {
				$this->delete_parents( $id );

			}
			redirect(admin_url('admin.php?page=parents&notice=delete_success'));
		}
	}
	...
	public function search_box( $text, $input_id ) {
		...
		<p class="search-box">
			...
			<input 
			    ...
			    name="student" 
			    placeholder="Nama/Nomor Siswa"
			    value="<?php echo $item['student']; ?>" 
		    />
		    <select 
			    type="search" 
