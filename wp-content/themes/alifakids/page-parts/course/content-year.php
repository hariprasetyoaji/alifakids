<?php 
    ...
    $query_years = $wpdb->get_results("
    	SELECT (select meta_value from {$wpdb->prefix}postmeta where post_id = p.ID and meta_key = 'year' limit 1) as year 
            ...
            AND (select meta_value from {$wpdb->prefix}postmeta where post_id = p.ID and meta_key = 'year' limit 1)
            GROUP BY p.ID 
    ");
	...
	<a class="card folder" href="<?php echo add_query_arg($url_args,site_url('/courses')); ?>">
	  <div class="card-body">
	      ...
	      <div class="folder-info">
	          <div class="folder-name"><?php echo $value ?></div>
	      </div>
	  </div>
	</a>
<?php endforeach ?>
