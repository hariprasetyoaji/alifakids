<?php 
    ...
    $query_years = $wpdb->get_results("
    	SELECT (select meta_value from {$wpdb->prefix}postmeta where post_id = p.ID and meta_key = 'day' limit 1) as day 
            ...
            AND (select meta_value from {$wpdb->prefix}postmeta where post_id = p.ID and meta_key = 'week' limit 1) = '".$_REQUEST['week']."'
            AND (select meta_value from {$wpdb->prefix}postmeta where post_id = p.ID and meta_key = 'day' limit 1)
            GROUP BY p.ID 
            ORDER BY day ASC
    ");
...
$icons_url = array( 
   ...
   '9' =>array(
	...
	if (is_parent()) {
		$available = [];
		$completed = getUserCourseDayReport($_REQUEST['student_id'], $_REQUEST['y'], $_REQUEST['month'], $_REQUEST['week']);

		if (!empty($completed)) {
			foreach ($completed as $value) {
				$available[] = intval($value);
			}
			$available[] = end($completed)+1;
		} else {
			$available[] = 1;
		}
...
foreach ($values as $key => $value): 
	...
	if (is_parent()) {
		...
		if ( in_array($value, $available) ) {
			$link = add_query_arg($url_args,site_url('/courses'));
		} else {
			$link = add_query_arg($url_args,site_url('/courses'));
			//$link = '#';
			//$disabled = true;
...
?>
	<a class="card folder <?php echo ($disabled) ? 'disabled' : '' ; ?>" href="<?php echo $link ?>">
	  <div class="card-body">
	      ...
	      <div class="folder-info">
	          	<div class="folder-name">
	          		Day <?php echo $value ?> 
	          		<?php if (!empty($completed) && in_array($value, $completed)) :  ?>
